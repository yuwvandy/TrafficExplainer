...
def compute_emb(model, pack_model, seq, mask, seq_byte_length, seq_byte_mask = None, baseline = None):
    """
    ...
    if seq_byte_mask == None:
        ...
        seq_emb = model.embed(seq.reshape(-1, seq_byte_length)) * seq_byte_mask[seq.reshape(-1, seq_byte_length)].unsqueeze(-1) # (batch, seq_len, d_model)
    ...
    seq_emb = seq_emb.mean(dim = 1).reshape(seq.shape[0], -1, seq_emb.shape[-1]) # (batch, packet_len, d_model)
...
def mix_collate_fn_pyg(batch):
    header_data, payload_data, target = list(zip(*batch))
    
    header_data = list(itertools.chain(*header_data))
    # print(len(header_data))
    payload_data = list(itertools.chain(*payload_data))
    
    print(header_data[0])
    header_data = Batch.from_data_list(header_data)
    payload_data = Batch.from_data_list(payload_data)
