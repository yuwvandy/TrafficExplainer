...
from utils import show_time, construct_graph, construct_graph_pyg, split_flow_Tor_nonoverlapping, split_flow_ISCX
from config import *
...
def construct_dataset_from_bytes_ISCX(dir_path_dict, type):
    ...
    for category in dir_path_dict:
        ...
        #split into 8:1:1
        data_list_train = data_list[:int(len(data_list) * 0.8)]
        data_list_val = data_list[int(len(data_list) * 0.8):int(len(data_list) * 0.9)]
        data_list_test = data_list[int(len(data_list) * 0.9):]


        train.extend(data_list_train)
        train_label.extend([category] * len(data_list_train))
        val.extend(data_list_val)
        val_label.extend([category] * len(data_list_val))
        test.extend(data_list_test)
        test_label.extend([category] * len(data_list_test))

        TRAIN_FLOW_COUNT[category] = len(data_list_train)
        VAL_FLOW_COUNT[category] = len(data_list_val)
        TEST_FLOW_COUNT[category] = len(data_list_test)

        print(TRAIN_FLOW_COUNT[category], VAL_FLOW_COUNT[category], TEST_FLOW_COUNT[category])

    if type == 'payload':
        np.savez_compressed(config.TRAIN_DATA, data=np.array(train), label=np.array(train_label))
        np.savez_compressed(config.VAL_DATA, data=np.array(val), label=np.array(val_label))
        ...
        np.savez_compressed(config.HEADER_VAL_DATA, data=np.array(val), label=np.array(val_label))
        np.savez_compressed(config.HEADER_TEST_DATA, data=np.array(test), label=np.array(test_label))

    print(TRAIN_FLOW_COUNT)
    print(VAL_FLOW_COUNT)
    print(TEST_FLOW_COUNT)

def construct_dataset_from_bytes_ISCX_ori(dir_path_dict, type):
    train = []
    train_label = []
    val = []
    val_label = []
    ...
    for category in dir_path_dict:
        ...
        data_list_val = data_list_train[:int(len(data_list_train)/5)]
...
def construct_graph_format_data_pyg(file_path, save_path, type, w_size=generalConfig.PMI_WINDOW_SIZE, pmi=1):
    ...
    for ind, p in enumerate(data):
        gs.append(construct_graph_pyg(bytes=p, w_size=w_size, k=pmi))
        if ind % 500 == 0:
            print('{} {} Graphs Constructed'.format(show_time(), ind))

    torch.save([gs, label], save_path)
...
if __name__ == '__main__':
    ...
    construct_graph_format_data_pyg(file_path=config.TRAIN_DATA, save_path=config.TRAIN_GRAPH_DATA, type='payload')
    construct_graph_format_data_pyg(file_path=config.VAL_DATA, save_path=config.VAL_GRAPH_DATA, type='payload')
    construct_graph_format_data_pyg(file_path=config.TEST_DATA, save_path=config.TEST_GRAPH_DATA, type='payload')

    construct_graph_format_data_pyg(file_path=config.HEADER_TRAIN_DATA, save_path=config.HEADER_TRAIN_GRAPH_DATA, type='header')
    construct_graph_format_data_pyg(file_path=config.HEADER_VAL_DATA, save_path=config.HEADER_VAL_GRAPH_DATA, type='header')
    construct_graph_format_data_pyg(file_path=config.HEADER_TEST_DATA, save_path=config.HEADER_TEST_GRAPH_DATA, type='header')
